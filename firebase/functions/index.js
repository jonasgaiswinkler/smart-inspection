...
const mailauth = require("./assets/mailauth.json");
...
exports.deleteObject = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    await admin
      .storage()
      .bucket()
      .deleteFiles({ prefix: `objects/${data.oid}/` });
    return { status: 200 };
  });

exports.deleteInspection = functions.https.onCall(async (data, context) => {
  if (context.auth && data.oid !== undefined && data.iid !== undefined) {
    await admin.firestore().collection("inspections").doc(data.iid).delete();
    const damagesSnap = await admin
      .firestore()
      .collection("objects")
      .doc(data.oid)
      .collection("damages")
      ...
      promises.push(
        (async function () {
          ...
          await admin
            ...
            .bucket()
            .deleteFiles({
              prefix: `objects/${data.oid}/damages/${damageDoc.id}/states/${data.iid}`,
            });
        })()
      );
    }
    ...
    await admin
      .storage()
      .bucket()
      .deleteFiles({ prefix: `inspections/${data.iid}` });
    return { status: 200 };
  } else {
    return { status: 401 };
  }
...
exports.deleteDamage = functions.https.onCall(async (data, context) => {
  if (context.auth && data.oid !== undefined && data.did !== undefined) {
    ...
    await admin
      .storage()
      .bucket()
      .deleteFiles({ prefix: `objects/${data.oid}/damages/${data.did}` });
    return { status: 200 };
  } else {
    return { status: 401 };
  }
      ...
      if (inspectionDoc.exists) {
          ...
          damagePromises.push(
            (async function () {
              const stateSnap = await db
                ...
                .get();
              const states = stateSnap.docs.map((doc) =>
        ...
        const doc = printer.createPdfKitDocument({
          ...
          pageBreakBefore(
            ...
            return (
              currentNode.pageNumbers.length > 1 && currentNode.unbreakable
            );
          },
        });
        const file = admin
          .storage()
          .bucket()
          .file("objects/" + data.oid + "/reports/" + rid + "/Bericht.pdf");
...
async function addObjectContent(
  content,
  locale,
  title,
  object,
  ...
  if (object.material) {
    table.push([
      locale.object.materials.name + ":",
      locale.object.materials.data[object.material],
    ]);
  }

  if (object.type) {
    table.push([
      locale.object.types.name + ":",
      locale.object.types.data[object.type],
    ]);
  }

  if (object.system) {
    table.push([
      locale.object.systems.name + ":",
      locale.object.systems.data[object.system],
    ]);
  }

  if (object.crossSectionShape) {
    table.push([
      locale.object.crossSectionShapes.name + ":",
      locale.object.crossSectionShapes.data[object.crossSectionShape],
    ]);
  }
  ...
  if (!assessmentSnap.empty) {
    ...
    addAssessmentContent(
      ...
      locale.assessment +
        " - " +
        locale.inspection.name +
        " " +
        DateTime.fromISO(assessment.date).toFormat("dd.MM.yyyy"),
      assessment
    );
    hr(content);
  }

  async function addObjectImage(imageKey, text) {
    const photo = await admin
      .storage()
      .bucket()
      .file("objects/" + oid + "/" + object[imageKey])
      .download();
    const photoStack = [];
    photoStack.push({
      text: text,
  ...
  if (object.photo) {
    await addObjectImage("photo", locale.object.photo);
  }

  if (object.groundPlan && object.imageGroundPlan) {
    await addObjectImage("imageGroundPlan", locale.damageGroundPlan);
  }

  if (object.longitudinalSection && object.imageLongitudinalSection) {
    await addObjectImage(
      "imageLongitudinalSection",
      locale.damageLongitudinalSection
    );
  }

  if (object.crossSection && object.imageCrossSection) {
    await addObjectImage("imageCrossSection", locale.damageCrossSection);
  }
}
...
function hr(content) {
  content.push({
    canvas: [
      { type: "line", x1: 0, y1: 0, x2: 595 - 2 * 50, y2: 0, lineWidth: 1 },
    ],
    margin: [0, 5],
  });
}

async function addInspectionContent(
  content,
  locale,
  title,
  inspection,
  ...
  content.push({
    text:
      ...
      locale.inspection.name +
      " " +
      DateTime.fromISO(inspection.date).toFormat("dd.MM.yyyy"),
    style: "header",
  });
  ...
  if (inspection.type) {
    table.push([
      locale.inspection.types.name + ":",
      locale.inspection.types.data[inspection.type],
    ]);
  }
  if (inspection.weather) {
    table.push([
      locale.inspection.weather.name + ":",
      locale.inspection.weather.data[inspection.weather],
    ]);
  }
  ...
  if (inspection.additionalInfo) {
    table.push([
      locale.inspection.additionalInfo + ":",
  ...
  if (inspection.photo) {
    const photo = await admin
      .storage()
      .bucket()
      .file("inspections/" + iid + "/" + inspection.photo)
      .download();
    const photoStack = [];
    photoStack.push({
...
async function addDamageContent(content, locale, title, damage, oid, iid, did) {
  ...
  if (currentState) {
    header +=
      ...
      locale.inspection.name +
      " " +
      DateTime.fromISO(currentState.date).toFormat("dd.MM.yyyy");
  }
  content.push({
    text: header,
    style: "header",
  ...
  if (damage.allocation) {
    tableData.push([
      locale.damage.allocations.name + ":",
      locale.damage.allocations.data[damage.allocation].name,
    ]);
  }

  if (damage.component) {
    tableData.push([
      locale.damage.component + ":",
      locale.damage.allocations.data[damage.allocation].data[damage.component],
    ]);
  }

  if (damage.componentDetail) {
    tableData.push([
      locale.damage.componentDetail + ":",
      damage.componentDetail,
    ]);
  }

  if (damage.type) {
    tableData.push([
      locale.damage.types.name + ":",
      locale.damage.types.data[damage.type],
    ]);
  }
  ...
  if (currentState) {
    ...
    for (const state of damage.states) {
      if (state.measurement1 && state.measurement1.value) {
        let row = [
          { text: DateTime.fromISO(state.date).toFormat("dd.MM.yyyy") },
          {
        ...
        if (
          currentState.limit &&
          toMillimetre(state.measurement1) >= toMillimetre(currentState.limit)
        ) {
          row[1].color = "#eb445a";
        }
        if (damage.measurement2Name && state.measurement2) {
          row.push({
            text: state.measurement2.value + " " + state.measurement2.unit,
          });
        }
    ...
    if (currentState.limit) {
      tableState.push([
        locale.damage.limitSm + ":",
        currentState.limit.value + " " + currentState.limit.unit,
      ]);
    }
    ...
    if (currentState.category) {
      tableState.push([
        locale.damage.categories.name + ":",
        locale.damage.categories.data[currentState.category],
      ]);
    }
  ...
  const addLocation = async function (location, text, file) {
    if (damage[location]) {
      const image = await admin
        .storage()
        .bucket()
        .file("objects/" + oid + "/damages/" + did + "/" + file + ".png")
        .download();
...
exports.deleteReport = functions.https.onCall(async (data, context) => {
  if (context.auth && data.oid && data.rid) {
    ...
    await admin
      .storage()
      .bucket()
      .deleteFiles({ prefix: `objects/${data.oid}/reports/${data.rid}` });
    return { status: 200 };
  } else {
    return { status: 400 };
  }
...
exports.notifyAdminLimit = functions.firestore
  .document("objects/{oid}/damages/{did}/states/{iid}")
  .onWrite(async (change, context) => {
    ...
    if (document !== null) {
      if (
        document.category === "immediateActionLimit" &&
        (oldDocument === null || document.category !== oldDocument.category)
      ) {
        ...
        if (admins.length !== 0) {
          ...
          const mailOptions = {
            ...
            bcc: emails,
            subject:
              "Soforteingriffsschwelle bei Objekt #" +
              context.params.oid +
              " festgelegt", // email subject
            html: `<p>Für den Mangel/Schaden #${
              context.params.did
            } wurde bei der Inspektion am ${new Date(
              document.date
            ).toLocaleDateString(
              "de-DE"
            )} die Soforteingriffsschwelle festgelegt.</p>
                <p><a href="https://jonasgaiswinkler.eu/smart-inspection/object/${
                  context.params.oid
                }/inspection/${document.date}/damage/${
              context.params.did
            }">Link zum Mangel/Schaden</a></p>
...
exports.notifyAdminAssessment = functions.firestore
  .document("objects/{oid}/assessments/{iid}")
  .onWrite(async (change, context) => {
    const document = change.after.exists ? change.after.data() : null;
    const oldDocument = change.before.exists ? change.before.data() : null;
    ...
    if (document !== null) {
      if (
        (document.substructure === 5 &&
          (oldDocument === null ||
            document.substructure !== oldDocument.substructure)) ||
        (document.superstructure === 5 &&
          (oldDocument === null ||
            document.superstructure !== oldDocument.superstructure)) ||
        (document.equipment === 5 &&
          (oldDocument === null ||
            document.equipment !== oldDocument.equipment)) ||
        (document.object === 5 &&
          (oldDocument === null || document.object !== oldDocument.object))
      ) {
        const users = await admin.auth().listUsers();
        const admins = users.users.filter(
          (user) =>
        ...
        if (admins.length !== 0) {
          ...
          const mailOptions = {
            ...
            bcc: emails,
            subject:
              "Schlechter Zustand bei Objekt #" +
              context.params.oid +
              " festgestellt", // email subject
            html: `<p>Für das Object #${
              context.params.oid
            } wurde bei der Inspektion am ${new Date(
              document.date
            ).toLocaleDateString(
              "de-DE"
            )} die Bewertung "Schlechter Zustand" festgelegt.</p>
                    <p>Unterbaubewertung: ${getAssessmentText(
                      document.substructure
                    )}</p>
                    <p>Überbaubewertung: ${getAssessmentText(
                      document.superstructure
                    )}</p>
                    <p>Ausrüstungsbewertung: ${getAssessmentText(
                      document.equipment
                    )}</p>
                    <p>Objektbewertung: ${getAssessmentText(
                      document.object
                    )}</p>
                <p><a href="https://jonasgaiswinkler.eu/smart-inspection/object/${
                  context.params.oid
                }/inspection/${document.date}">Link zur Inspektion</a></p>
                <p>Hinweis: Bitte nicht auf diese E-Mail antworten!</p>`,
          };
