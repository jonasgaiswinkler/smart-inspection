...
export default {
    ...
    actions: {
        async save(context: any) {
            ...
            const objectDoc = db.collection("objects").doc();
            await objectDoc.set({
            ...
            if (params.photo != null) {
                const photoPromise = storage.ref("/objects/" + objectDoc.id + "/photo" + getFileExtension(params.photo)).put(params.photo);
                promises.push(photoPromise);
            }
            if (params.groundPlan != null) {
                const groundPlanPromise = storage.ref("/objects/" + objectDoc.id + "/groundPlan" + getFileExtension(params.groundPlan)).put(params.groundPlan);
                promises.push(groundPlanPromise);
            }
            if (params.longitudinalSection != null) {
                const longitudinalSectionPromise = storage.ref("/objects/" + objectDoc.id + "/longitudinalSection" + getFileExtension(params.longitudinalSection)).put(params.longitudinalSection);
                promises.push(longitudinalSectionPromise);
            }
            if (params.crossSection != null) {
                const crossSectionPromise = storage.ref("/objects/" + objectDoc.id + "/crossSection" + getFileExtension(params.crossSection)).put(params.crossSection);
                promises.push(crossSectionPromise);
            }
            if (params.techDescription != null) {
                const techDescriptionPromise = storage.ref("/objects/" + objectDoc.id + "/techDescription" + getFileExtension(params.techDescription)).put(params.techDescription);
                promises.push(techDescriptionPromise);
            }
            if (params.model != null) {
                const modelPromise = storage.ref("/objects/" + objectDoc.id + "/model" + getFileExtension(params.model)).put(params.model);
                promises.push(modelPromise);
            }

            if (params.files != null) {
                Object.keys(params.files).map(function (key: any) {
                    const filePromise = storage.ref("/objects/" + objectDoc.id + "/files/" + params.files[key].name).put(params.files[key]);
